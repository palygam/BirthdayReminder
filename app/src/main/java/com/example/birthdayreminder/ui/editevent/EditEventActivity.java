...
public class EditEventActivity extends BaseActivity implements EditEventActivityView {
    ...
    public void displayDatePickerDialog(int year, int month, int day) {
        DatePickerDialog datePicker = new DatePickerDialog(EditEventActivity.this, (view, thisYear, monthOfYear, dayOfMonth) -> {
            presenter.onDateSet(thisYear, monthOfYear, dayOfMonth);
            date = presenter.onCalendarSet(thisYear, monthOfYear, dayOfMonth).getTimeInMillis();
        }, year, month, day);
        datePicker.show();
    }
